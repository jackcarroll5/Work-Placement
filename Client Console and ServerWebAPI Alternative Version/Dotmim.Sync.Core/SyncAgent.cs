...
namespace Dotmim.Sync
{
    ...
    public class SyncAgent : IDisposable
    {
        ...
        public int ApplyChangedFailed { get; set; }
        ...
        public async Task<SyncContext> SynchronizeAsync(SyncType syncType, CancellationToken cancellationToken, IProgress<ProgressArgs> progress = null)
        {
            ...
            try
            {
                ...
                // 1) Read scope info
                // ----------------------------------------
                // get the scope from local provider - Client
                ...
                // get the scope from remote provider - Server
                ...
                // ----------------------------------------
                // Get Schema from remote provider - Server
                ...
                // Apply on local Provider - Client
            ...
            finally
            {
                ...
                context = await this.RemoteProvider.EndSessionAsync(context);//Server
                context = await this.LocalProvider.EndSessionAsync(context);//Client
